...
**I7 — `SeriesRepository` 817 → 585.** The first file taken off the over-600 list, and the one the
project's own notes name: it was itself an extraction that “moved the problem instead of solving
it”. Split by RESPONSIBILITY, not by line ranges — [`SeriesDetailFetcher`] owns one series' seasons
and episodes (the whole memory → disk → network → fallback chain), [`SeriesCategoryHealth`] owns
which categories the provider is failing on and the cooldown that hands back the last good list
instead of an empty one. Bodies verbatim, signatures unchanged, and `KotlinFilesOver600Lines`
lowered 16 → 15 in the SAME commit, which is what locks the gain in.

**Still open after this tier, and why:** 15 Kotlin files over 600 lines (the ratchet stops growth;
...
- **Never put media3-session (`androidx.media3:media3-session`) on the player** (2026-09-02). Its legacy
  ...
  remote pause, reproduced on `.64`. The session is the framework `android.media.session.MediaSession`
  behind `PlayerMediaSessionManager`; an "upgrade" to media3-session must first prove `adb shell media
  dispatch pause` does not kill the process on the Fire TV.
...
| 2026-08-04 | **Tier H batch 2 — H2 silent same-file failover (`53ab5899`).** New `ui/sources/SourceAlternates.kt` shared by movies + series: `next(source, triedCount)` swaps delivery (directSource/headers/provider/type/name/cache) onto a copy that keeps the stream identity incl. `stream_id`; budget `MAX_ALTERNATE_ATTEMPTS=2`, separate from the different-file `MAX_AUTO_FALLBACK_ATTEMPTS=3`; `AlternateFailover` holds the per-controller retry state (kept out of the at-ceiling controllers). Both `handlePlayerResult`s now try an alternate BEFORE blacklist + NOT_CACHED — the replay is silent (no toast, no auto-fallback budget). Series wiring inline hit CCM-15 → split into `retriedViaAlternate`, **not baselined**. 6 unit tests (swap/budget/unplayable-skip/exhaust-reset/per-file/unknown). Device sanity on .64: debrid CW resume unaffected (~16-23 Mbps HEVC, clean exit). Ledger stays 8. **Next: H3 honest languages.** | **8** | **1** | 0 | 43 | `53ab5899` |

| 2026-08-04 | **Tier H batch 3 — H3 honest languages (`e34f3e41`).** LanguageParser stops inventing: blank/unmatched → `emptyList()`, no "en" injection into MULTI-only; adapter renders honestly-unknown as ONE neutral "🌐 ?" chip; **MULTI now matches any specific language filter** (without this the English filter would HIDE rows the fake "en" used to admit) and Unknown genuinely reaches unknown rows; tiering agreed across BOTH sorters (SourceSorter exact-claimed 100 > MULTI 60; FilterUtils 3 > 1 — the sheet sorts twice); the in-player picker now passes `preferredAudioLang` (was "ALL"). Tests: 3 parser assertions rewritten + 2 sorter tiers + new 4-test filter suite. Device QA on .64: honest "HI · UNK" chips (no fake EN), MULTI distinct, EN-priority order intact — **and H1's "×2" addonCount badges visible on device for the first time**; 79 sources all cache-verdicted. Ledger stays 8. **Tier H remaining: H4 learned languages (Room v15 + migration test) → H5 Firestore share → H6 addon reliability.** | **8** | **1** | 0 | 43 | `e34f3e41` |

| 2026-08-04 | **Tier H batch 4 — H4 learned languages (`4ed7e7b2`); MULTI finally becomes real chips.** The player is now the ground truth: on `onTracksChanged` EVERY audio-track language (not just selected) is normalised ISO-639-2→1 (hin→hi, deu/ger→de, `und` dropped) and stored once per source in the new `release_languages` table keyed by `stableStreamIdentity` (debrid-only by construction; merge-on-replay so a codec-limited session never erases knowledge). **Room v14→v15**: additive `MIGRATION_14_15` + exported v15 schema + `migrate14To15` instrumented test — **all 5 migration tests PASS on the Fire TV** (incl. the never-destructive guards). Overlay at the `UnifiedSourceProvider` facade (both movie + episode paths): learned languages replace claimed chips + `languagesVerified`; miss/failure degrades to claimed. UI: ✓ on the FIRST chip (strip clips right); tiering in BOTH sorters: **verified 100 > claimed 80 > MULTI 60** (FilterUtils 4 > 3 > 1). Tests: sorter retiered + 3 normalization tests. Device QA on .64, full loop live: played the Spider-Man CW release → **"H4 learned languages: [en, hi, ta, te]"** → reopened picker → **"H4 overlay: 3/74 sources got verified languages"** → the three copies of the played file show "EN ✓ HI …" and sort to the very top under EN priority. Logcat clean. Ledger stays 8. **Tier H remaining: H5 Firestore share → H6 addon reliability.** | **8** | **1** | 0 | 43 | `4ed7e7b2` |

| 2026-08-04 | **Tier H batch 5 — H5 shared learned languages (`d497a83b`); one user's playback teaches every device.** New `ReleaseLanguageCloudSync`: publish = fire-and-forget set(merge) with languages as a **MAP** (merge = native set-union, codes only widen — also kills the SpreadOperator arrayUnion needed); fetch = whereIn(documentId) ≤10 per chunk, failures → empty. **Only 40-hex infoHashes sync** (title.hashCode ids are instance-specific and would poison the table). Repository: recordVerified also publishes; recordShared (LOCAL always outranks); fetchAndCacheShared persists origin=shared. Overlay asks the community about the top-30 unknown hashes under `withTimeoutOrNull(2500)` — the sheet never waits. **Rules DEPLOYED (debridxtream-new) and verified live via REST**: SDK-shape commit ACCEPTED, open read OK, extra-key 403, non-40-hex-id 403; test doc admin-deleted. Device QA on .64: H5 lookup in-path with zero regression ("H4 overlay 3/74" unchanged, no fetch errors, no FATAL); a hashless-id play correctly SKIPPED publish. Two new-file detekt hits fixed at source (NBD chunk helper, map-merge vs spread), **not baselined** — ledger stays 8. Follow-up noted: StremThru "DIR" rows keep the hash only in the URL (stream_id = title.hashCode) so they don't share; teaching streamIdFor the H0 URL-parse widens coverage but changes saved-resume identity → own batch. **Tier H remaining: H6 addon reliability (+ optional H5b URL-hash stream ids).** | **8** | **1** | 0 | 43 | `d497a83b` |

| 2026-08-04 | **Tier H batch 6 — H5b URL-hash stream ids (`3224ad99`); the H5 loop closes for proxy rows.** `streamIdFor` parses the hash out of DIR-proxy URLs with H0's `urlHashAndIdx` before the title-hash fallback (id now agrees with the H0/H1 grouping identity); `streamMatchesStableIdentity` learns the same parse so scoped resume matches the NEW saved ids, while the title-hash branch keeps pre-H5b history matching. Resume audit: playback resume is play-then-repair on the saved URL (never a stream_id list lookup); every other stream_id comparison is within one list build. +3 StreamIdentityTest cases. Device QA on .64 — **the full H5 round-trip finally proven live**: played a StremThru Torz row → learned `[en]` under a REAL 40-hex (`11f8fe22…42f1`, was title-hash before) → **the shared Firestore doc exists ({en}, reports=2, server timestamp), readable by every device**; CW resume kept working across the id change (14:14→17:22→18:11 over three sessions); logcat clean. Ledger stays 8. **Tier H remaining: H6 addon reliability.** | **8** | **1** | 0 | 43 | `3224ad99` |

| 2026-08-04 | **Tier H batch 7 — H6 addon reliability (`91351624`); TIER H (H0-H6) COMPLETE.** New `AddonReliabilityStore` (persistent cousin of SessionSourcePreference): per provider-FAMILY success/failure in SharedPreferences, Laplace `(s+1)/(s+f+2)`, **NEUTRAL until 5 observations**, counts halve past 200; family = label head before ':' case-folded, derived identically on record (player provider extra) and score (providerName extra ?: source label) sides. Success = first rendered frame (once per source attempt); failure = terminal death that NEVER produced a frame (a 90-minute-in stall is a network story — not counted). Wired as `groupStreamsByFile`'s reliability hook (H1's waiting parameter) in both full paths — FIRST same-file tiebreak ahead of metadataScore; never re-ranks different files, never hides a row. Store loaded off-main at MainActivity launch. Tests: +5 store (neutral gate/Laplace/ordering/family/unknown) +2 grouping (reliability outranks metadata; neutral untouched). Device QA on .64: played the learned StremThru row — now "**EN ✓" AND the BEST top pick** (H4/H5b visibly compounding) — first frame rendered, `addon_reliability.xml` holds `stremthru torz = 1,0`, logcat clean. Ledger stays 8. | **8** | **1** | 0 | 43 | `91351624` |

| 2026-08-04 | **Tier H batch 8 — H8 deterministic language-chip pattern (`83a6a01f`), owner-reported: 3+ languages clipped arbitrarily after two chips.** Per row: **preferred audio language leads** (StreamLanguage parse-equality — "Hindi"/"HI"/"hin" all match), max 2 codes render, the rest collapse to a **"+N" counter**; on overflow rows only the FIRST chip keeps its flag emoji (strip stays inside its 52dp column); the H4 ✓ stays on the first (guaranteed-visible) chip; container contentDescription carries the FULL list. `setPreferredLanguage` wired from both sheet hosts (movie controller + shared movie/series bottom sheet). Bonus: ta/te/ml/kn/pa/bn/mr/gu/ur added to FLAG_EMOJI_BY_CODE — Tamil rendered "UNK" before. Device QA on .64 (HI priority), text-level via uiautomator: verified row "🇮🇳 HI ✓ EN +2" (desc "Verified languages: Hindi, English, Tamil, Telugu"), claimed 3-lang rows "🇮🇳 HI TA +2", 1-2-lang rows unchanged; logcat clean. Ledger stays 8. | **8** | **1** | 0 | 43 | `83a6a01f` |

| 2026-08-04 | **Tier H batch 9 — H9 dual language priority + full settings vocabulary (`a5277896`), owner-requested.** (1) Priority selector now offers EN HI DE FR IT ES PT RU NL PL TR UR PA TA TE ML KN (+MULTI/ALL); PORTUGUESE/DUTCH/POLISH/TURKISH added to StreamLanguage. (2) New **"Secondary Audio Language"** setting (default NONE) — "Hindi na mile to German": both sorters tier **primary verified 100 > primary claimed 80 > secondary verified 75 > secondary claimed 70 > MULTI 60** (FilterUtils 6/5/4/3/1); a secondary-only row used to score ZERO and sink below MULTI. Sheet header shows the pair ("DE+HI PRIORITY"); in-player picker passes it too. (3) Chips: BOTH priorities lead the strip — any priority language a row carries is NAMED, never buried in "+N". Bonus: the facade sort read a never-written SettingsPreferences key (language-blind) — now the same CredentialsPreferences pair as the sheets. CCM-16 on apply split into resolveSortLanguages, not baselined. +4 sorter tests. Installed on .64, 0 FATAL; **the owner set DE+HI via the new settings row minutes after install** (prefs prove the dialog works in the wild); ranked-sheet visual spot-check deferred — the TV went into live viewing mid-QA. Ledger stays 8. | **8** | **1** | 0 | 43 | `a5277896` |

| 2026-08-04 | **Discover catalog fix (`0f5ceb2f` + `01e88723`), owner-reported "Region select kiya, content bahut thora".** Three stacked causes: (1) the Stremio Discover grid never called the ViewModel's existing `loadNextPage()` — scroll listener + auto-fill-to-40 added; (2) `loadContent`'s `catch(Exception)` **swallowed CancellationException** — a filter change's `loadJob.cancel()` ran the dead job's catch AFTER the new load's reset and stamped `canLoadMore=false` onto fresh state, freezing the grid at ~1-2 pages forever (the documented rethrow-CE landmine; DebridSeeAllViewModel had the same swallow, both fixed); (3) the vote_count≥200 floor measured against TMDB live: **ALL of Bollywood = 107 titles, Tamil = 5, Punjabi = ZERO** — regional grids now use floors popular 10 / top-rated 50 (hi → ~1.2k titles), the global no-language grid keeps 200 (softcore guard). Device QA on .64: Region=Bollywood + 20 pages of scrolling — fresh titles kept flowing incl. deep-catalog 1971 entries, no stall, no FATAL. **Bonus same session: H9's deferred visual check completed — "85 sources · sorted by DE+HI priority" header live on The Odyssey sheet.** Ledger stays 8. | **8** | **1** | 0 | 43 | `01e88723` |

| 2026-09-02 | **Whole-project world-standard audit + batch A (`8cba2a2d`).** Measured, not quoted: baseline 8 = ceiling, 824 unit + 47 instrumented methods, secrets untracked, shipped APK = release R8 (panel/APK agree), every OkHttp client bounded. **Fixed:** `network_security_config` trusted USER CAs in base-config — any proxy CA on a customer device could read the RD token + Xtream creds; now system-only, user store in `<debug-overrides>` (verified in the shipped APK's xmltree, cold-start to Home on .64, TMDB https OK, 4K playback, 0 FATAL/SSL). `check_repo_hygiene.sh` anchored its junk regex to the path START, so subdirectories were invisible — 64 committed captures/build logs untracked, `scratch/` may never be tracked. CLAUDE.md's "we ship DEBUG / R8 crashes" paragraph rewritten (release has shipped since vC 73). 2 TV ImageViews re-labelled `@null`. **Open, recorded in the session memory `world-standard-audit-2026-09-02`, not here:** file-size creep the ratchet cannot see (16 files >600 lines; `BasePlayerFragment` 1491→1771), 337 strings untranslated in all 5 locales, no MediaSession / Watch Next / parental PIN, no CI, Tier G + F1 never started, E14 never measured on the release build. | 8 | 1 | 0 | 47 | `8cba2a2d` |
