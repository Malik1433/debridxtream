...
## Successful Pattern - Premium 4K Debrid Playback RAM & Tunneling Strategy
- **Date**: 2026-06-26
- **Achievement**: Resolved infinite buffering and stuttering for high-bitrate 4K Debrid streams.
- **Key Implementation**: Neutralized eMMC I/O choking by completely bypassing physical disk caching (`CacheDataSource.Factory`) in `PlayerActivity.kt` when `isDebrid` is true. Elevated the ExoPlayer RAM ceiling to 128MB in `PlayerBufferConfigFactory.kt` and applied strict 30s-60s pre-buffering thresholds. Finally, activated hardware Video Tunneling (`setTunnelingEnabled(true)`) via `DefaultTrackSelector` to guarantee perfect audio/video sync handled by the device's hardware composer.
- **Proof**: Code implemented and deployed successfully to device. User confirmed "ok issue resolved".
...
### Smart Focus Navigation for Player Controls
- **Date**: 2026-06-27
- **Change**: Implemented "Smart Focus" routing for the ExoPlayer controller overlay. When the overlay is brought up with DPAD_CENTER/UP/DOWN, focus snaps perfectly to the primary `exo_play`/`exo_pause` control. If the overlay is invoked using DPAD_LEFT/RIGHT, it bypasses the transport buttons and jumps straight to the timeline `exo_progress` bar for immediate seeking. Ensured high-contrast (3dp/4dp borders) focused states for the custom selector drawables, fixed layout XML focus routing to remove jumps to off-screen elements, and implemented a proper BACK key interception that gracefully dismisses the player overlay before bubbling up to the activity.
