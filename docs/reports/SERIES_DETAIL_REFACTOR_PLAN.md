...
- **SD-4b `SeriesDebridSourceController`** (~450 lines, <600): `fetchAndShowDebridSources`, `iptvGroupsForEpisode`, `verifyIptvGroups`, `refreshMediaFusionCacheStatus`, `updateRdSummary` + `formatSizeLabel`, `resolveDebridEpisodeFetchContext`, `pickResumeSource`, `pickNextDebridEpisodeSource`, `markDebridEpisodeSourceCached`, `consumeDebridReturnFocusStreamIds` + the seven maps. Calls back to playback via `onPlayIptvSource` / `onPlayDebridSource` (the current `onSourceSelected` `sourceType=="IPTV"` branch, preserved). Reads `seriesSeasonUi.selectedSeasonKey/selectedEpisode` + metadata getters. **QA focus: source sheet populates, cache-status badges refresh, IPTV cross-source rows appear** (verifiable via the `source_list_loaded` diagnostics JSONL).
- **SD-4c `SeriesDebridPlaybackController`** (~400 lines, <600): `playDebridEpisode`, `playEpisode`, `resumeDebridEpisode`, `playIptvSourceFromDebrid`, `autoPlayNextDebridEpisodeSource`, `shouldTryNextDebridSource`, `notifyDebridFailure`, and the `playerLauncher` `registerForActivityResult` (register from the controller during `onCreate`, before STARTED) + `autoFallbackAttempts` / `openedFromPlaybackFailure`. Reads state via the SD-4b controller. **QA focus: first frame on debrid + IPTV-from-debrid, resume position, source-failover auto-advance, return-to-sources after a failed stream** (the known landmines — must be eyes-on).
