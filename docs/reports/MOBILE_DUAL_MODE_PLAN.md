...
|---|---|---|---|---|
| 2026-08-14 | **P3** | **The Detail page becomes a phone screen — built to the owner's `design_handoff_dx_play_detail`.** The handoff's five decisions, and where each one is: (a) the header is **poster-led and fixed at 232dp** — a 16:9 backdrop hero would cost 231dp before a word of metadata and is missing often enough that the screen would have two shapes, so the backdrop is blurred atmosphere behind the block and a poster-derived gradient fills the same 232dp when there is none, with no layout branch; (b) **one filled Play/Resume button** with the progress bar welded into its bottom edge, over up to four 64dp actions that re-flex rather than ever showing a dead button; (c) **300 episodes** get a sticky season bar and a season SHEET carrying each season's watched progress, with the episode list opening on NEXT UP; (d) **60 sources are a screen, not a sheet** — filter-first, every chip carrying its count, BEST pinned above the filters where no filter can hide it; (e) the plot clamps to four lines and **More expands it in place**. There is no bottom navigation here: Detail is a pushed destination, and dropping the bar returns 64dp to the content, which is what keeps Play above the fold. Architecturally the screen is a `PhoneDetailView` that renders a `PhoneDetailModel`, and each host translates into that model rather than the screen learning four dialects — so a fix to the header or the episode row lands on every path at once. `DetailScreens` is the single door every caller now goes through, the same shape as `SectionNavigator.createHomeFragment`. **Stated plainly, because it is half the paths: this batch covers the IPTV movie and series detail** (the Movies and Series tabs, Search, and the IPTV cards on Home). **The DEBRID detail still opens the old landscape activity** — it is a 786-line collaborator-driven Activity that also owns the source resolver, and half-wiring it would have been worse than leaving it whole. The Sources screen is built and waiting for it | **Phone (Pixel emulator, portrait, real IPTV data) — PASS, measured.** Movies tab → a title → the portrait detail: header 609px = **232dp**, poster 273x410 = **104x156dp**, primary button 147px = **56dp**, action row 168px = **64dp** — every number the handoff fixes. Real content: title, rating, `2026 · NC-17`, the full plot with a working **More**, a Cast rail, and the honest `NO SIMILAR TITLES` line rather than an empty carousel. The action row carried Favourite and Watched and **no Sources button** (IPTV has no source list) and **no Trailer** (this title has none) — the row flexes rather than showing either. Play started PlayerActivity, which then bounced on this emulator's dead stream, and BACK returned to the grid. **One real defect came out of the device and is fixed here:** the muted no-cast line was built in code with `ResourcesCompat.getFont(share_tech_mono)` and threw `Resources$NotFoundException` — that font is DOWNLOADABLE, a trap this project has hit before, so the line is inflated from XML like every other screen. **TV on the Fire TV, on the release build, since every detail caller was re-routed:** the debrid movie detail still opens `MovieDetailActivity` (`tv_title`, `DEBRID / MOVIES`, IMDb row) and the IPTV series detail still opens the TV fragment (`rv_episodes`, `section_episodes`) — no `phone_dt_*` id anywhere on the television. **0 FATAL on both.** Unit tests and detekt green | *(this commit)* |
| 2026-08-14 | **P2** | **Live TV becomes a phone screen — built to the owner's `design_handoff_dx_play_live_tv`, and it is the handoff's three decisions that make it different from the TV one.** (a) **One tap plays into a docked 16:9 player**, not fullscreen: Live TV is a zapping loop, and fullscreen-per-tap charges a back press per hop while hiding the list the user is hunting through. `PhoneLiveDock` pins the video under the app bar and the channel list keeps scrolling underneath; fullscreen stays deliberate (the surface, the expand button, or rotating), and BACK collapses the dock before it leaves the screen (MainActivity priority 0b — Live is reached through the back stack, so without that the pop would walk out while a channel was still playing). (b) **Categories are pinned 48dp chips plus an All-N bottom sheet with its own search** — three or four categories carry daily use and the other 145 are near-never. (c) **The guide is per channel**, opened from the now/next line: a timeline grid is unreadable at 411dp and `EpgGridView` implements `onKeyDown` and nothing else, so shipping it to a touchscreen would ship a screen that renders and then ignores every finger. Playback is NOT reimplemented: the dock drives `PreviewPlayerPanel`, the same collaborator the TV preview uses, by supplying the same view ids — the tuned live player and its AudioTrack-releasing error handling were expensive to get right. Rows are 88dp and paged (a category here can hold 2000). Four things came out of QA rather than the design, and the first is a real defect: **`EpgCache` is a process-wide object that answers with nothing until a screen hands it a repository, and the only screen that ever did so was the TV's `LiveFragment`** — so the phone list read NO GUIDE DATA with 102,798 programmes sitting in the database, the lookup returning before it ever reached them. Then: the warm is asynchronous and nothing re-bound the rows when it landed (now a debounced re-bind on `EpgCache.updates`); this playlist numbers every channel **0**, so the number and the logo-fallback tile both printed "0" (the number hides, the tile falls back to the channel's initials); and logos arrive as portal-relative paths, so they go through `toAbsoluteUrl` like everywhere else. Fixed inline: Settings offered **Live TV Layout** on a phone, a classic-vs-guide choice that now changes nothing there. Also honoured: the existing **Resume last channel** setting, which the TV guide already obeys — on a phone it starts the last channel in the dock instead of only offering a card | **Phone (Pixel emulator, 1080x2400, portrait, real playlist) — PASS on everything the provider allowed.** The screen renders with real data: chips `All categories 366` / `WORLD CUP 2026 37`, an 88dp row per channel (measured 231px = 88dp), bottom nav with Live active. **One tap plays**: tapping a row opens the 231dp dock with the channel name, an `HD` badge and the header switching to `TAP TO ZAP`. **BACK collapses the dock and does not leave** — first BACK removed the dock and brought the resume card back with the list intact, second BACK returned to Home. **Search is the system keyboard** (`mInputShown=true`), typing `bein` filtered to the beIN channels and the header read `CHANNELS · 50`. **The favourite star writes through**: starring a live row took Home's Favorites row from 1 to 2 with `|WC| BEIN SPORTS 1 HD FR` in it. The **no-EPG** case is the one this playlist actually has, and it degrades exactly as specified — full 88dp row, `NO GUIDE DATA · TAP TO PLAY`, empty progress track, no NEXT. The **error panel** was exercised for real: switching to a category that needed a fetch returned `HTTP 401 Unauthorized`, and the list area showed the amber panel with Retry / Use cached while the chrome above stayed put. Settings no longer offers **Live TV Layout** on a phone. **Two defects came out of this run and both are fixed in this commit** (see the description). **Not verified here, and stated rather than implied: live video and the EPG-present rows/guide.** This emulator's stored Xtream credentials are being rejected — `ChannelPagingSource: retrofit2.HttpException: HTTP 401` — so no stream can start on it, and although a forced XMLTV sync did land 102,798 programmes, none of them are for the cached World Cup channels (`beinsports1.fr`, `dazn1.es` all resolve to `current=null, next=null`). Those paths need the owner's own phone. **TV verification on the Fire TV (.64), on the RELEASE build, because SectionNavigator, MainActivity's BACK and SettingsFragment are shared:** Home unchanged with launch focus on `btn_hero_watch`; the rail opens the **TV's own classic Live screen** (`preview_player_view`, `preview_tile`, `300 CH`, the EPG day tabs) — the phone screen does not leak onto the television; TV Settings still lists **Live TV Layout / New EPG Guide**, so the gate is phone-only. **0 FATAL on both.** Unit tests and detekt green | *(this commit)* |
| 2026-08-14 | **P1** | **The phone Home is its own screen now, in PORTRAIT — the first screen with a real handset design rather than the TV's layout at 1.6x.** Built to the owner's `design_handoff_dx_play_home`: 56dp app bar (wordmark + 48dp search + account avatar), a 452dp hero rotating every 7s, then Continue Watching → Movies → Series → Recent Live Channels → Favorites, over a 64dp bottom bar whose five destinations re-weight to four when Debrid is not entitled. Three decisions worth stating: (a) `PhoneHomeFragment` is a SEPARATE fragment from `HomeFragment` — the two no longer share an arrangement, so sharing one file would mean two designs fighting over one set of views; (b) they still share the DATA and the RULES — `HomeNavigationRouter` now takes a `HomeRouterHost` interface instead of `HomeFragment`, so the play-then-repair resume and every detail route stay in ONE place and were not copied; (c) **orientation follows the fragment on top** (`PortraitScreen` marker + a `FragmentLifecycleCallbacks` watcher), because one Activity hosts every browse screen and only Home has a portrait design — Live, Movies, Series, Search and Settings keep the landscape they were built for until their own design lands. The 1.6x font scale therefore STAYS on the Activity and this screen opts out locally (`unscaledInflater`), which also preserves the user's own accessibility scale. Favorites reads the existing `FavoriteDao` rather than widening `HomeUiState` for one screen, and the hero bookmark writes to that same table. Also built to spec: the loading skeleton at real card geometry, the Favorites empty panel, a 16:9 channel tile for Recent Live (a channel logo in a 2:3 poster frame was mostly empty tile), and the hero's meta/quality chips hidden rather than rendered as an empty pill — the catalogue has no runtime, year or codec | **Phone (Pixel emulator, 1080x2400, 420dpi, portrait, real data) — PASS.** All five rows render with real content: hero "Spider-Man: Brand New Day" ★7.9 with a 2-line title, Continue Watching 2 (`1:12:44 / 1:41:12` with its progress bar), Movies 10, Series 10, Recent Live 1 (`|WC| BEIN SPORTS 1 HD FR` in a 208x117dp tile), Favorites 0 with the dashed panel. Every target measured at or above 48dp; nothing below 12sp; content starts at y=63px, i.e. below the status bar. **One tap acts, proved four ways:** More Info → MovieDetailActivity ("Spider-Man: Brand New Day"), avatar → Settings, bottom-nav Movies → the browse grid, and a Continue Watching card → `RESUME_PATH: DIRECT to PlayerActivity`. The hero bookmark toggled and the Favorites row went 0 → 1 live. **Orientation ladder verified both directions:** Home portrait (rotation 0) → Movies landscape (rotation 1) → BACK → Home portrait again; the player opens landscape. **TV verification on the Fire TV (.64), on the RELEASE build, since the router the TV uses was refactored:** the TV Home is unchanged — hero "Spider-Man: Brand New Day" with its dots, "Continue Watching / 4 TITLES", launch focus on `btn_hero_watch`, and none of the `phone_*` ids present. D-pad DOWN/LEFT/RIGHT move focus, the rail opens Live TV with real EPG ("ON AIR NOW · Tennis : Tournoi WTA du Canada"), a poster opens MovieDetailActivity, BACK returns home, and **a Continue Watching card resumes into PlayerActivity playing 1920x1080 at ~1.9Mbps**. **0 FATAL on both.** Not verified: a real handset (emulator only) | *(this commit)* |
| 2026-08-10 | **D8** | **The Debrid home loses its hero on the phone — owner decision, from a photo of his real handset.** The photo showed the hero owning ~70% of the screen with ONE row peeking at the bottom (D2/D3's 250dp trim helped but could not fix a band that is chrome on a page whose value is the rows). The mechanism already existed: `applyHeroVisibility` — the same path Discover uses. New `debrid_home_shows_hero` bool (touch **false** / TV **true**) gates it, so on a phone `showHero` is always false and on TV the expression reduces to exactly the old `key != "discover"`. The hero-hidden content margin became a qualified dimen (`stremio_content_top_no_hero` 70/**52**dp — 70 because D3 measured the nav bar's 67dp floor; 52 is the code's old Discover literal) and the with-hero branch now reads the existing `stremio_content_top` dimen (TV 318dp = the old inline literal). The rotation timer is gated too (`onResume` + the visibility path), which as a side-effect KILLED the "uiautomator never idles on the Debrid home" trap on the phone — the hero was the rotator | **Phone (Pixel emulator): PASS, measured.** The Debrid home opens straight into content: top nav, then "Trending Movies" with full-height cards and "Trending Series" below — no hero anywhere; `heroBanner`/`heroTitle`/`heroPlayBtn` at 0 occurrences in the tree and `content_host` at y=184px = **70dp exactly**; and `uiautomator dump` WORKS on this screen now. **TV: verified by construction + aapt2 + walk-through** — `(television) true` makes the gate the old expression verbatim and the margin the old 318dp literal (`aapt2`: false/**true**, 70/**52**dp); the vC 67 build was walked on .64 through home, Settings (v2.5.0 visible), a Debrid movie detail and a Debrid series detail, all unchanged. The Debrid HOME itself was not hand-reached on the TV this round (blind D-pad rail navigation kept landing on cards) — same construct-verified caveat D6 recorded | *(this commit)* |
| 2026-08-10 | **M20** | **The SELECT SOURCE panel for the hand — the LAST §1e batch — plus an orphan deleted on the owner's word.** Phase 1 (66cf94eb): `MediaFusionConfigActivity` was in the manifest but launched by NOTHING in the codebase, and despite its name it saved Stremio addon URLs — Settings → Addons' job. Owner said delete; gone with its layout and five orphan strings (−327 lines); the MediaFusion fetcher/prefs/entitlement gate live on untouched. Phase 2, the panel: it renders inside the detail activities so the 1.6x scale already reaches its px÷2 type — what it had was the same §1c classes as everywhere else. New `dimens_sources.xml` pair (TV literals verbatim): the panel top sat under the phone's status bar (`source_panel_top_inset` 24/**0**dp), the header 60/**44**dp, the three filter chips 48/**22**dp, source rows 48/**34**dp, and the pill boxes sized for unscaled type that clip at 1.6x — provider 48×28/**34×20**, quality 52/**38**, type 64/**48**, badges 24/**17**dp, play circle 32/**22**dp, and the panel itself 480/**410**dp because the TV's fixed columns left the release name ~5 characters at phone scale. One stale surprise: `values/dimens.xml` already held an UNUSED `source_row_height` 60dp from an abandoned "Source Picker Redesign" — deleted, the new pair owns the name | **Phone (Pixel emulator, real data — 10 sources on a Trending film): PASS, measured.** Panel 1260px = 480dp with the header starting at y=63px = 24dp (clear of the status bar), filter chips **126px = 48dp**, row pitch 126px = 48dp, provider 126×74px = 48×28dp, quality 137px = 52dp, type 168px = 64dp, play circle 84px = 32dp — every pill whole, nothing clipped, and **one tap on a row → PlayerActivity**. **TV smoke on .64: PASS, measured** — the panel opens at **820px = 410dp** with its header 88px = **44dp at y=0**, "SELECT SOURCE · DE+HI PRIORITY" compact as before, and the no-sources empty state renders its message. `aapt2` shows every dimen carrying `(television)` at the exact old literal (48/**22**, 24/**0**, 48/**34**, 480/**410**…). **§1e IS NOW COMPLETE: M13-M20 + D1-D7 all shipped** | *(this commit)* |
