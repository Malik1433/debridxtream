...
| ~~P21~~ | ✅ **DONE (`8753c5c`)** — `PlayerLiveEpgRenderer` (144): renderOverlay + bindNextSlot + the 15 now/next views (owns its own findViewById via bindViews). epgOverlay container + channel-meta views + epgOverlayUi stay (PiP/OSD/C5 share them); a thin Activity `renderOverlay` forwarder keeps C5 unchanged. baseline net 0 (351). Device .64: live plays, EPG binds via OSD path, no crash (legacy strip doesnt draw when liveOsd active). | 2238 | done |
| ~~P22~~ | ✅ **DONE (`f768ebf`)** — `PlayerStallMonitor` (209): the 5s poll loop + checkForStall + checkVideoRenderProgress (audio-plays-video-frozen landmine: live re-prepare / one-shot tunneling-off reinit) + readyStallRequiredStrikes + the 11 stall/freeze consts. Owns stallDetector/freezeDetector; reactions hand off to recovery/liveTuner. Thin Activity start/stopStallMonitor forwarders keep P20/liveTuner/lifecycle callers unchanged. baseline net 0 (351). Device .64: playback + first frame clean (monitor wires at every play start). | 2087 | done |
| ~~P23~~ | ✅ **DONE (`6c5375f`)** — `PlayerMetadataBinder` (79): bindModernMetadata (poster/cleaned-title/S1·E2·SOURCE subtitle/quality pill). Views looked up on PlayerView per call (never fields). Thin Activity forwarder keeps the ~4 callers unchanged. baseline net 0 (351). Device .64: playback + first frame clean. | 2047 | done |
| ~~P24~~ | ✅ **DONE (`6973a08`)** — `PlayerMediaItemFactory` (78): buildMediaItem — MIME hint + subtitle configs + the TS-vs-HLS LiveConfiguration gate (the landmine moved as a UNIT, byte-identical). maybeMatchDisplayFrameRate stayed (window/display concern, not media-item). Thin Activity buildMediaItem forwarder + diagnosticsFieldsWithMime host. detekt clean (no baseline change). Device .64: live TS first frame + 3-zap all fresh first frames, NO freeze. | 2013 | done |
| ~~P25~~ | ✅ **DONE (`b37b198`)** — `PlayerLaunchArgs` (54, object): the ~30 intent→session identity assignments out of onCreate via `readInto(activity, session)` using `with(session)` (byte-identical; EXTRA keys stay on PlayerActivity as the createIntent contract). detekt clean (no baseline change). Device .64: VOD (type=MOVIE debrid, first frame) + live TS (first frame) launch both clean. | 1982 | done |
...
| ~~P27-b~~ | ✅ **DONE (`3633cab`)** — the four live-only branches of `onViewCreated` moved verbatim into four `protected open` hooks (`setupLiveOverlayChrome`/`observeLiveEpgAndState`/`startLiveZappingAndBrowser`/`seedLiveEpgOverlay`) that ONLY `LivePlayerFragment` overrides; a VOD instance runs the empty base impls so the `contentType != LIVE_TV` path is byte-identical. Bumped `private`→`internal`: `viewModelBinder`, `liveChannelIds`, `setupOverlayViews()`, `initBrowserOverlay()`. The live collaborator FIELDS (liveTuner/liveOsd/epgRenderer/channelBrowser/zapDebouncer) deliberately STAY in the base — shared recovery/stall/PiP/overlay/lifecycle paths reference them → **base line-count flat (1628→1628)**; the reduction lands when a later P27 step moves the fields + shared-player adopt/hand-off + live key routing down. compile+tests+detekt green, baseline untouched. LivePlayerFragment 17→51. **Device-verified .64 (adb-driven + logcat):** live TS first-frame + sustained Amlogic AVC decode, zap ×2 (fresh decoder surface each), channel-browser open+BACK (player stays), live exit→guide, VOD episode resume (VodPlayerFragment cinematic loader→playback advancing, controller/metadata OK); zero app exceptions. OSD/EPG/PiP visuals not screencap-able on the secure surface but their paths ran crash-free. Pushed to main. | 1628 | done |
| ~~P27-c~~ | ✅ **DONE (`62ac5dd`)** — the live-only branch of the `onBackPressedDispatcher` callback (OSD drawer → channel browser → EPG-overlay dismiss) moved into `protected open fun handleLiveBack(): Boolean` (base default `false`); only `LivePlayerFragment` overrides it, so a VOD instance falls straight through to VOD-controller/exit — byte-identical (the old inline checks were already no-ops without liveOsd/browserState). **KEY FINDING that reshaped the roadmap:** the live collaborator FIELDS (liveTuner/liveOsd/zapDebouncer/epgRenderer) are read by ~8 sibling collaborators via `activity.<field>` (EventListener/InputRouter/StallMonitor/ViewModelBinder/DebridCoordinator/SeriesController/LiveTuner/EpgRenderer), all typed on `BasePlayerFragment` — so the fields CANNOT move to the subclass without breaking those (they serve shared paths too). Base ≤800 therefore comes from moving live METHOD BODIES down, not the fields; the fields stay declared in the base. compile+tests+detekt green, baseline untouched. Device-verified .64: live BACK closes the channel browser & stays in the player, then exits; zero exceptions. Base 1628→1627. | 1627 | done |
| ~~P27-d~~ | ✅ **DONE (`5a06009`)** — the live+shared EXIT hand-off moved into `LivePlayerFragment`: `handBackSharedPlayerIfNeeded` (private) + the `performBackExit` live branch behind `protected open fun performLiveSharedExitIfNeeded(): Boolean` (base default false → VOD takes plain releasePlayer+finish, byte-identical). Bumped private→internal: sharedLiveSession, sharedExitInProgress, playerListener, debugListener. The ADOPT half (inside initializePlayer) deliberately stayed (untouched → no adopt risk) — that's P27-e. compile+tests+detekt green, baseline untouched. **Device-verified .64 incl. the landmine:** enabled the playback-diagnostics marker + drove the EPG-guide grow→fullscreen→BACK cycle; the session jsonl shows `player_adopted_shared_tuned` + `first_frame_rendered` + `player_handed_back_shared` + clean `session_finished`, zero app exceptions. Base 1627→1598, Live 51→106. Pushed. | 1598 | done |
| ~~P27-e~~ | ✅ **DONE (`4e208a7`)** — the LP-ADOPT block inside `initializePlayer` moved into `LivePlayerFragment.adoptSharedLivePlayerIfPresent` behind `protected open fun adoptSharedLivePlayerIfPresent(streamUrl): Boolean` (base false). `initializePlayer` now guards cold init with `if (!adoptSharedLivePlayerIfPresent(streamUrl)) { …cold init… }` — byte-identical for VOD. Bumped private→internal: didPlaybackComplete, hasAppliedIndexOverride, hasRenderedFirstFrameForCurrentSource, frameRateMatchedForCurrentSource; dropped the now-unused base AudioAttributes import. compile+tests+detekt green, baseline untouched. **Device-verified .64 via diagnostics jsonl:** shared adopt writes `player_adopted_shared_tuned`(+handed_back on BACK); a Debrid-movie Resume writes `player_initialize_started`+`media_item_built`+`player_prepare_called`+`first_frame_rendered` (4K @ ~9.9 Mbps) — both the adopt and cold-init paths verified. Base 1598→1567, Live 106→145. Pushed. | 1567 | done |
