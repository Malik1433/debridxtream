...
| ~~P21~~ | ✅ **DONE (`<p21>`)** — `PlayerLiveEpgRenderer` (144): renderOverlay + bindNextSlot + the 15 now/next views (owns its own findViewById via bindViews). epgOverlay container + channel-meta views + epgOverlayUi stay (PiP/OSD/C5 share them); a thin Activity `renderOverlay` forwarder keeps C5 unchanged. baseline net 0 (351). Device .64: live plays, EPG binds via OSD path, no crash (legacy strip doesnt draw when liveOsd active). | 2238 | done |
