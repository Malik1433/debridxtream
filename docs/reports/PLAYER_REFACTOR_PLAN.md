...
| P27-e | Move the live shared-player ADOPT half (the `LiveSharedPlayer.adopt`/`takeFrame`/`showAdoptedCoverFrame` block inside `initializePlayer`) into `LivePlayerFragment` behind an open hook returning the adopted player (or null). Same diagnostics-file QA (`player_adopted_shared_tuned`). | Base ~1560 | HIGH (live) |
