...
| P27-d | Move the live shared-player adopt/hand-off cluster (`LiveSharedPlayer.adopt`/`takeFrame`/`showAdoptedCoverFrame` + `handBackSharedPlayerIfNeeded`/`offer` + the live half of `performBackExit`) into `LivePlayerFragment`. **LANDMINE (hand-off order) — needs the mini↔fullscreen EPG-guide launch QA path.** | Base ~1560 | HIGH (live) |
