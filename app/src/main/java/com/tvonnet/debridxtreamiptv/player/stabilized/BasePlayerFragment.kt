...
open class BasePlayerFragment : Fragment(), PlayerRecoveryController.RecoveryHost {
    ...
    override fun onPause() { super.onPause(); if (::historyManager.isInitialized) historyManager.saveProgressSnapshot(); wasPlayingBeforePause = player?.isPlaying == true; Log.i("PlayerActivity", "fragment onPause, wasPlaying=$wasPlayingBeforePause") /* F1 Phase 0 */; player?.pause(); timeoutHandler.removeCallbacks(timeoutRunnable); stopStallMonitor() }
    override fun onStop() {
