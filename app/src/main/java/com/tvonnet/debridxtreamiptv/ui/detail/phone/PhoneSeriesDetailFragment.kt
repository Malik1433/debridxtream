...
class PhoneSeriesDetailFragment : Fragment(), PortraitScreen {
     ...
     */
    /** The episode's stream picker, which owns its own dialog. */
    private val streams: PhoneEpisodeStreamSheet by lazy {
        PhoneEpisodeStreamSheet(
            this,
            onPick = { option -> viewModel.onStreamSelected(option) },
        ).also { sheet -> sheet.onDismiss { viewModel.closeStreamPanel() } }
    }
