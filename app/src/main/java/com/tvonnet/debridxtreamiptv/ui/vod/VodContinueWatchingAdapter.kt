...
/**
 * Horizontal "Continue Watching" rail for the VOD Movies screen.
 * Reuses the shared [ContinueWatchingItem] model produced by WatchHistoryPreferences,
 * rendered with the cinematic landscape card [R.layout.item_vod_cw_card].
 */
class VodContinueWatchingAdapter(
    private val onItemClick: (ContinueWatchingItem) -> Unit
) : ListAdapter<ContinueWatchingItem, VodContinueWatchingAdapter.CwViewHolder>(CW_DIFF) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CwViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.item_vod_cw_card, parent, false)
